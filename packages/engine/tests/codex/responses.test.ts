...
import { codexGenerateText, normalizeError } from "../../src/agent/codex-bridge.js";
...
describe("codex reasoning continuity", () => {
  ...
  it("replays opaque reasoning in output order across tool steps without sharing it outside the call", async () => {
    const reasoning = (id: string) => ({
      type: "reasoning",
      id,
      summary: [{ type: "summary_text", text: "Check the supplied training details." }],
    ...
    const secondOutput = [reasoning("rs_third"), call("third")];
    ...
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(response(firstOutput))
      .mockResolvedValueOnce(response(secondOutput))
      .mockResolvedValueOnce(okResp(TEXT_EVENTS))
      .mockResolvedValueOnce(okResp(TEXT_EVENTS));
    const execute = vi.fn(async () => "training details");
    const deltas: string[] = [];
    const messages = [{ role: "user" as const, content: "Review my training." }];
    const opts = {
      messages,
      modelId: "gpt-5.4",
      profileName: "openai-codex",
      tools: {
        read_training: { inputSchema: zodSchema(z.object({ kind: z.literal("recent") })), execute },
      },
      onTextDelta: (delta: string) => deltas.push(delta),
    };
    const ports = { getAccessToken: async () => TOKEN, classifyFailure };

    const result = await codexGenerateText(opts, ports);
    await codexGenerateText(opts, ports);
    ...
    const userInput = {
      role: "user",
      content: [{ type: "input_text", text: "Review my training." }],
    };
    const toolOutput = (call_id: string) => ({
    ...
    expect(bodies[3].input).toEqual([userInput]);
    expect(bodies.every((body) => body.store === false)).toBe(true);
    expect(messages).toEqual([{ role: "user", content: "Review my training." }]);
    ...
    expect(result).toMatchObject({
      ...
      finishReason: "stop",
      steps: 3,
      totalUsage: { inputTokens: 36, outputTokens: 9, cachedInputTokens: 6, totalTokens: 45 },
    });
  });
});
